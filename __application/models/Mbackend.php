	...
	function getdata($type="", $balikan="", $p1="", $p2="",$p3="",$p4=""){
		...
		switch($type){
				...
				if($mod=='rekap_penjualan'){
					...
					if(count($res)>0){
						foreach($res as $x=>$v){
							...
							$sql="SELECT A.*,B.no_order,CONCAT(D.nama_sekolah,' (',D.npsn,')')as sekolah,C.judul_produk
									FROM tbl_d_pemesanan A
									LEFT JOIN tbl_h_pemesanan B ON A.tbl_h_pemesanan_id=B.id
			...
			case "tbl_produk":
				...
				$sql="SELECT A.*,D.nama_kategori, E.tingkatan 
					FROM tbl_produk A
					LEFT JOIN cl_kategori D ON A.cl_kategori_id = D.id 
					LEFT JOIN cl_tingkatan E ON A.cl_tingkatan_id = E.id ".$where;
					//echo $sql;
			break;
			...
			case "tbl_foto_produk":
				if($balikan=='row_array'){
					 ...
					 $where .=" AND A.tbl_produk_id=".$this->input->post('id');
				}
				$sql="SELECT A.*, E.nama_kategori,F.tingkatan 
					FROM tbl_foto_produk A 
					LEFT JOIN tbl_produk B ON A.tbl_produk_id=B.id
					LEFT JOIN cl_kategori E ON B.cl_kategori_id=E.id 
					LEFT JOIN cl_tingkatan F ON B.cl_tingkatan_id = F.id ".$where;
			break;
			case "l_kelas":
	...
	function simpandata($table,$data,$sts_crud){ //$sts_crud --> STATUS NYEE INSERT, UPDATE, DELETE
		...
		switch($table){
			...
			case "tbl_produk":
				...
				if($sts_crud=='delete'){
					$data_foto = $this->getdata('tbl_foto_produk','result_array');
					if(count($data_foto)>0){
						foreach($data_foto as $v){
							if(isset($v['foto_produk'])){
								$path='__repository/produk/'; //.$v['tingkatan'].'/'.$v['kelas'].'/'.$v['nama_group'].'/'.$v['nama_kategori'].'/';
								chmod($path.$v['foto_produk'],0777);
								unlink($path.$v['foto_produk']);
								$this->mbackend->simpandata('tbl_foto_produk',$v,'delete');
							}
						}
					}
				}
